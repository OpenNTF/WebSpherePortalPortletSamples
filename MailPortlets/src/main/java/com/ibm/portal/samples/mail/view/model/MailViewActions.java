 ...
 * 
 * Action implementations modify the underlying {@link MailViewModel} which will
 ...
 * the {@link MailViewActions#commit()} method.
 * 
 * @author cleue
 */
public class MailViewActions implements Committable, Disposable {
	...
	public enum ACTION {
		...
		REPLY {
			 ...
			 * 
			 * @see
			 * com.ibm.portal.samples.portlettemplate.model.MailViewActions.
			 * ACTION
			 * #processAction(com.ibm.portal.samples.portlettemplate.model.
			 * MailViewActions.ActionEntry)
			 */
			@Override
	...
	private final class ActionEntry {
		 ...
		 */
		private final boolean processReply() throws MessagingException {
			// logging support
			final String LOG_METHOD = "processReply()";
			if (bIsLogging) {
				LOGGER.entering(LOG_CLASS, LOG_METHOD);
			}
			// access the current message bean
			final MessageBean message = model.getSelectedMessage();
			if (message != null) {
				// prepare the data transfer
				final SendEventBean replyBean = new SendEventBean();
				replyBean.setReceiver(message.getSenderMail());
				replyBean.setSubject("Re: " + message.getSubject());
				// send
				response.setEvent(SendEventBean.EVENT_NAME, replyBean);
			} else {
				// log this
				if (bIsLogging) {
					LOGGER.logp(LOG_LEVEL, LOG_CLASS, LOG_METHOD,
	...
	private static final String LOG_CLASS = MailViewActions.class.getName();
	 ...
	 *            the dependencies
	 */
	public MailViewActions(final MailViewModel aModel,
		...
		assert aDeps != null;
		// logging support
		final String LOG_METHOD = "MailViewActions(aModel, aRequest, aResponse, aDeps)";
		if (bIsLogging) {
			LOGGER.entering(LOG_CLASS, LOG_METHOD);
		}
